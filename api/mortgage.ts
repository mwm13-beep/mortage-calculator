...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method !== "POST") {
    return sendError(req, 405, "METHOD_NOT_ALLOWED", { msg: "POST required" });
  }

  // Basic content-type check
  const ctype = (req.headers.get("content-type") || "").toLowerCase();
  if (!ctype.startsWith("application/json")) {
    return sendError(req, 415, "UNSUPPORTED_MEDIA_TYPE", {
      msg: "Content-Type must be application/json",
    });
  }

  // Very small payload guard (optional; header may be absent)
  const len = Number(req.headers.get("content-length") || 0);
  if (len && len > 10_000) {
    return sendError(req, 413, "PAYLOAD_TOO_LARGE");
  }
  ...
  try {
    ...
    // Best-effort client identifier (Edge has no socket)
    ...
    if (!success) {
      return sendError(req, 429, "TOO_MANY_REQUESTS");
    }

    // ---- Parse/validate with dynamic context ----
    // Edge Request.json() is safe; still validate via Zod schema
    ...
    const ui = validated.data;
    const result = computeResultsDynamic(ui);
